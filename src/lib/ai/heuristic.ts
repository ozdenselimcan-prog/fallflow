...
export function detectBuildingType(text: string): string {
  ...
  if (/einfamilien|\befh\b/.test(t)) return BUILDING_TYPES[0];
  return "";
}
...
export function heuristicExtract(text: string): Record<string, string> {
  ...
  const year = text.match(/(?:baujahr|gebaut|aus dem jahr|von)\D{0,12}(1[6-9]\d{2}|20[0-4]\d)/i)?.[1];
  ...
  set("name", text.match(/(?:ich bin|mein name ist|ich heiße)\s+(\p{Lu}[\p{L}-]+(?:\s+\p{Lu}[\p{L}-]+)?)/u)?.[1]);
